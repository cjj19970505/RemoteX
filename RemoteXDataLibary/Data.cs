...
/// <summary>
/// 这个是专门用来做数据处理的
/// 做一份，Host和手机app都用这个库就好
/// </summary>
namespace RemoteXDataLibary
{
    ...
    public struct Data
    {
        ...
        private static int typeToCount(int sensorType)
        {
            switch (sensorType)
            {
                ...
                case (int)DataType.TouchMouseSpeed:
                    return 2;
                case (int)DataType.OrientationAngle:
                    return 3;
        ...
        public static Data FromString(string sData)
        {
            string[] tempSs = sData.Split('|');
            sData = sData.Remove(0, 1);
            sData = sData.Remove(sData.Length - 1, 1);
            string[] sTypeAndData = sData.Split('|');
            ...
            Data restoredData = new Data(type, values);
            return restoredData;
        }
